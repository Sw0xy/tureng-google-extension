...
export const Popup = () => {
    ...
    return (
        <div className="w-[370px] h-[500px] bg-bg m-0 p-0 flex flex-col items-center">
           <h1 className="text-3xl text-white font-light my-4 flex items-center"><p className="text-red-500">tur</p>eng</h1>
            <form onSubmit={e => handleSearch(e)} className="w-[350px] rounded-md flex items-center mx-auto shadow-md shadow-light_gray">
                <div className="bg-dark_gray p-1 flex items-center gap-x-2 w-full rounded-md focus-within:outline-none focus-within:border-light_gray focus-within:ring-2 focus-within:ring-light_gray">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-12 h-12 text-gray-300 mx-1">
                            ...
                            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-dark_gray py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                                {langs.map((item, i) => (
                                    ...
                                    >
                                        {({ selected }) => (
                                            <>
                                                <span className={`block truncate text-center mx-auto text-sm ${selected ? 'font-medium text-white' : 'font-normal'}`} >
                                                    {item.name}
                                                </span>
                                            </>
                                        )}
                    ...
                    <button type="submit" className="text-gray-500 ml-5 hidden"></button>
                </div>
            </form>
            <div className="w-full max-w-[350px] overflow-auto custom-scroll h-4/5 flex flex-col gap-y-2 mt-2">
                ...
                }
                {
                    meanings === null ? <></> : meanings.map((item, i) => (
                        <div key={i} className="flex items-center w-full text-sm bg-dark_gray rounded  hover:bg-light_gray">
